...
export default function TreasureMineField(){
  ...
  useOffscreenAnimationClock(hostRef, { sceneId: "treasure-mine" });
  const { animationsEnabled,animationsPaused,ultraLite,isMobile }=useAnimationPreferences();
  useEffect(()=>{const host=hostRef.current,baseCanvas=baseCanvasRef.current,fxCanvas=fxCanvasRef.current;if(!host||!baseCanvas||!fxCanvas)return undefined;const baseCtx=baseCanvas.getContext("2d",{alpha:true}),fxCtx=fxCanvas.getContext("2d",{alpha:true,desynchronized:true});if(!baseCtx||!fxCtx)return undefined;let destroyed=false,width=1,height=1,dpr=1,resizeFrame=0,wakeTimer=0,pageVisible=!document.hidden;const cover=document.createElement("canvas");
    const paintFinalBase=(now=performance.now())=>{renderMineBase(baseCtx,width,height,1,now,cover);completedRef.current=true;};
    const resize=()=>{resizeFrame=0;const rect=host.getBoundingClientRect(),nextWidth=Math.max(1,Math.round(rect.width)),nextHeight=Math.max(1,Math.round(rect.height)),nextDpr=Math.min(window.devicePixelRatio||1,(ultraLite||isMobile)?1:1.4);if(nextWidth===width&&nextHeight===height&&nextDpr===dpr&&baseCanvas.width>1)return;width=nextWidth;height=nextHeight;dpr=nextDpr;for(const canvas of [baseCanvas,fxCanvas]){const pixelWidth=Math.round(width*dpr),pixelHeight=Math.round(height*dpr);if(canvas.width!==pixelWidth)canvas.width=pixelWidth;if(canvas.height!==pixelHeight)canvas.height=pixelHeight;canvas.style.width=`${width}px`;canvas.style.height=`${height}px`;}cover.width=width;cover.height=height;baseCtx.setTransform(dpr,0,0,dpr,0,0);fxCtx.setTransform(dpr,0,0,dpr,0,0);if(completedRef.current)paintFinalBase();else renderMineBase(baseCtx,width,height,activeRef.current?1:.02,performance.now(),cover);renderMineFx(fxCtx,width,height,completedRef.current?1:.02,performance.now());};
    const scheduleResize=()=>{if(resizeFrame)return;resizeFrame=requestAnimationFrame(resize);};
    const scheduleFrame=(delayMs=0)=>{window.clearTimeout(wakeTimer);const request=()=>{wakeTimer=0;if(destroyed||!activeRef.current||!pageVisible||rafRef.current)return;rafRef.current=requestAnimationFrame(frame);};if(delayMs>4)wakeTimer=window.setTimeout(request,Math.max(0,delayMs-4));else request();};
    const frame=now=>{rafRef.current=0;if(destroyed||!activeRef.current||!pageVisible||animationsPaused)return;if(!startedAtRef.current)startedAtRef.current=now;const duration=ultraLite||isMobile||!animationsEnabled?1:960,reveal=completedRef.current?1:(duration<=1?1:clamp((now-startedAtRef.current)/duration));if(!completedRef.current){renderMineBase(baseCtx,width,height,reveal,now,cover);if(reveal>=1)completedRef.current=true;}const runtimeQuality=document.documentElement.dataset.runtimeQuality||"high",fxFps=animationsEnabled&&!isMobile?resolveMineFxFps(runtimeQuality,ultraLite):0,fxInterval=fxFps>0?1000/fxFps:0;if(fxFps>0&&now-lastFxRef.current>=fxInterval){lastFxRef.current=now;renderMineFx(fxCtx,width,height,reveal,now);}else if(fxFps===0){renderMineFx(fxCtx,width,height,reveal,now);}if(reveal<1)scheduleFrame();else if(fxFps>0)scheduleFrame(Math.max(0,fxInterval-(now-lastFxRef.current)));};
    const start=()=>{activeRef.current=true;startedAtRef.current=completedRef.current?performance.now():0;lastFxRef.current=0;window.clearTimeout(wakeTimer);cancelAnimationFrame(rafRef.current);rafRef.current=0;if(pageVisible)scheduleFrame();};
    const stop=()=>{activeRef.current=false;startedAtRef.current=0;window.clearTimeout(wakeTimer);cancelAnimationFrame(rafRef.current);rafRef.current=0;fxCtx.clearRect(0,0,width,height);};
    const handleVisibility=()=>{pageVisible=!document.hidden;if(pageVisible&&activeRef.current)scheduleFrame();else{window.clearTimeout(wakeTimer);cancelAnimationFrame(rafRef.current);rafRef.current=0;}};
    const observer=new IntersectionObserver(entries=>{const entry=entries[0];if(entry?.isIntersecting&&entry.intersectionRatio>.10)start();else stop();},{threshold:[0,.10,.30]});observer.observe(host);const ro=typeof ResizeObserver!=="undefined"?new ResizeObserver(scheduleResize):null;ro?.observe(host);window.addEventListener("resize",scheduleResize,{passive:true});document.addEventListener("visibilitychange",handleVisibility);resize();
    return()=>{destroyed=true;observer.disconnect();ro?.disconnect();window.removeEventListener("resize",scheduleResize);document.removeEventListener("visibilitychange",handleVisibility);window.clearTimeout(wakeTimer);cancelAnimationFrame(rafRef.current);cancelAnimationFrame(resizeFrame);};
  },[animationsEnabled,animationsPaused,ultraLite,isMobile]);
  return <div ref={hostRef} className="treasure-mine-field" aria-hidden="true" data-mine-field="excavation-runtime" data-render-mode="static-base-dynamic-fx" data-mobile-render-mode={isMobile ? "single-frame-static" : "dynamic-fx"}><canvas ref={baseCanvasRef} className="treasure-mine-canvas treasure-mine-base-canvas"/><canvas ref={fxCanvasRef} className="treasure-mine-canvas treasure-mine-fx-canvas"/><span className="mine-shaft-light"/><span className="mine-dust-overlay"/></div>;
}
