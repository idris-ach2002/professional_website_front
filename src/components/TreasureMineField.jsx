...
export default function TreasureMineField(){
  ...
  useEffect(()=>{const host=hostRef.current,baseCanvas=baseCanvasRef.current,fxCanvas=fxCanvasRef.current;if(!host||!baseCanvas||!fxCanvas)return undefined;const baseCtx=baseCanvas.getContext("2d",{alpha:true}),fxCtx=fxCanvas.getContext("2d",{alpha:true,desynchronized:true});if(!baseCtx||!fxCtx)return undefined;let destroyed=false,width=1,height=1,dpr=1;const cover=document.createElement("canvas");
    ...
    const resize=()=>{const rect=host.getBoundingClientRect();width=Math.max(1,Math.round(rect.width));height=Math.max(1,Math.round(rect.height));dpr=Math.min(window.devicePixelRatio||1,(ultraLite||isMobile)?1:1.4);for(const canvas of [baseCanvas,fxCanvas]){canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);canvas.style.width=`${width}px`;canvas.style.height=`${height}px`;}cover.width=width;cover.height=height;baseCtx.setTransform(dpr,0,0,dpr,0,0);fxCtx.setTransform(dpr,0,0,dpr,0,0);if(completedRef.current)paintFinalBase();else renderMineBase(baseCtx,width,height,activeRef.current?1:.02,performance.now(),cover);renderMineFx(fxCtx,width,height,completedRef.current?1:.02,performance.now());};
    const frame=now=>{if(destroyed)return;if(!activeRef.current||animationsPaused){rafRef.current=0;return;}if(!startedAtRef.current)startedAtRef.current=now;const duration=ultraLite||isMobile||!animationsEnabled?1:960,reveal=completedRef.current?1:(duration<=1?1:clamp((now-startedAtRef.current)/duration));if(!completedRef.current){renderMineBase(baseCtx,width,height,reveal,now,cover);if(reveal>=1)completedRef.current=true;}const runtimeQuality=document.documentElement.dataset.runtimeQuality||"high",fxFps=animationsEnabled&&!isMobile?resolveMineFxFps(runtimeQuality,ultraLite):0;if(fxFps>0&&now-lastFxRef.current>=1000/fxFps){lastFxRef.current=now;renderMineFx(fxCtx,width,height,reveal,now);}else if(fxFps===0){renderMineFx(fxCtx,width,height,reveal,now);}if(reveal<1||fxFps>0)rafRef.current=requestAnimationFrame(frame);else rafRef.current=0;};
