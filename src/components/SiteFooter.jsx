...
export default function SiteFooter({ owner }) {
  const contacts = (owner?.contacts ?? []).slice(0, 4);

  return (
    <footer className="simple-footer" aria-label="Coordonnées de fin de page">
      <Text className="simple-footer-name">{getOwnerFullName(owner)}</Text>
      <Group justify="center" gap="xs" className="simple-footer-links">
        {contacts.map((contact) => (
          <Anchor key={`${contact.type}-${contact.value}`} href={getContactHref(contact)} target={contact.type === "EMAIL" || contact.type === "PHONE_NUMBER" ? undefined : "_blank"}>
